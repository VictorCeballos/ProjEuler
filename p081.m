...
%matrix = load('data/p081_simple.txt');
matrix = load('data/p081_matrix.txt');
N = size(matrix,1);
...
while (true)
    ...
    lineCount = fprintf(['visiting node (%d,%d). Progress: %d/%d %s %.2f%%\n'], ...
        ...
        N*N, char(8776), sum(visited(:))/(N*N)*100);
    ...
    % and assign the smaller one. For example, if the current node A is marked
    ...
    % back to step 3.
    [~, index] = min(tentative_distance(:) + visited(:) * sum(matrix(:)));
